...
import modal from "~/components/department/modal.vue";
import { useDepartmentStore } from "~/store/department";
definePageMeta({
  middleware: "alc",
  moduleKey: moduleKey.DEPARTMENT,
});
...
const getData = async () => {
  ...
  await ifetch(departmentAPI.get, input).then((res) => {
    setInput(res);
  });
};
  ...
  swal({
    ...
    preConfirm: () => {
      return new Promise(async (resolve) => {
        await ifetch(departmentAPI.delete, { id: id });
        iAlert().success();
        getData();

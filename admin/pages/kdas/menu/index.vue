<template>
  ...
  const getData = async () => {
    ...
    try {
      const data = await ifetch(KDASMenuAPI.get, input);
    ...
    swal({
      ...
      preConfirm: () => {
        return new Promise(async (resolve) => {
          let input = { id: row.id};
          const data = await ifetch(KDASMenuAPI.delete, input);
          iAlert().success();
          // setInput(data);
          getData();
          resolve();
  ...
  const handleStatusChange = async (value, row, reload) => {
    if (row.id) {
      await ifetch(KDASMenuAPI.updateStatus, {
        id: row.id,
        status: value ? 1 : 0,
      });
      getData();

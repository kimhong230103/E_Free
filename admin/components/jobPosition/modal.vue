      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">
            {{ $t(props.actionType) }} {{ $t("job_position") }}
          </h5>
        </div>
          ...
          <div class="row">
            <div class="col-12">
              ...
              <select
                ...
                @change="v$.department_id.$touch"
              >
                <option :value="null">{{ $t("please_select") }}</option>
                <option
                  v-for="(item, index) in useDepartment.getList"
                  :value="item.id"
                  :key="index"
                >
              ...
              <span
                ...
                v-if="v$.department_id.$error"
              >
                {{
                  `${$t("department")} ${$t(
                    v$.department_id.$errors[0].$message
                  )}`
                }}
              </span>
            </div>
...
import { departmentAPI, jobPositionAPI } from "~/constants/api";
...
onMounted(() => {
  ...
  state.modal = new bootstrap.Modal("#departmentModal");
});
...
const save = async () => {
  ...
  let url =
    props.actionType == appConst.formAction.add
      ? jobPositionAPI.store
      : jobPositionAPI.update;
...
const showModal = async (rowId = null) => {
  if (rowId) {
    ...
    form.id = rowId.id;
    form.department_id = rowId.department_id;
  }

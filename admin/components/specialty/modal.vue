<template>
  <div class="modal fade" tabindex="-1" id="specialtyModal">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">
            {{ $t(props.actionType) }} {{ $t("specialty") }}
          </h5>
        </div>
          ...
          <div class="row">
            <div class="col-12">
              ...
              >
                ...
                <input
                  v-model="form.temp_name[index].specialty"
                  type="text"
                  class="form-control"
                  :id="`name-${item.code}`"
                  :placeholder="`${$t('name')} (${item.code.toUpperCase()})`"
                  :class="{
                    'is-invalid': v$.temp_name[index].specialty.$error,
                  }"
                  @change="v$.temp_name[index].specialty.$touch"
                  required
                  autocomplete="off"
                />
                <span
                  class="invalid-feedback"
                  v-if="v$.temp_name[index].specialty.$error"
                >
                  {{
                    $t("name") +
                    " (" +
                    ...
                    ") " +
                    $t(v$.temp_name[index].specialty.$errors[0].$message)
                  }}
                </span>
              </div>
            </div>
...
import { specialtyAPI } from "~/constants/api";
...
onMounted(() => {
  ...
  state.modal = new bootstrap.Modal("#specialtyModal");
});
...
const save = async () => {
  ...
  const nameMap = form.temp_name.reduce((acc, item) => {
    acc[item.language_code] = item.specialty;
    return acc;
  }, {});
  const input = {
...
const showModal = async (rowId = null) => {
  if (rowId) {
    ...
    form.temp_name = defaultForm.temp_name.map((defaultItem) => ({
      ...defaultItem,
      specialty: parsedName[defaultItem.language_code] || null,
    }));
    form.id = rowId.id;
  }

...
<!-- Note -->
<!-- 
  #. when row.status = 0 (background-color: red) (defalut)
  #. If we want custom bg color for row, just add (bgColor) to every row
          example: 
                    const itemsFilter = rows.map(row=>{
                      if(row.status===1) return {...row, bgColor:"green"}
                    })
                    // pass itemsFilter to IFormTable
-->

<template>
  <div class="card" :class="{ 'border-none': showBorder }">
    <div class="card-body">
      <div
        :style="{ height: props.height, maxHeight: '68vh' }"
        class="table-responsive table-striped table-container"
      >
        <table
          :class="{
            'table-striped': true,
        ...
        >
            ...
            >
              ...
              <td
                ...
                :style="{
                  width: obj?.width ? obj?.width : '150px',
                  textAlign: obj?.textAlign ? obj?.textAlign : 'left',
                  color: item.status === 0 ? '#000000 !important' : 'normal',
                }"
              >
                <div
                  :class="obj?.classes"
          ...
          <!-- <tfoot>
            <tr>
              <th v-for="obj in showColumn.list.filter((item) => !item.hide || item.hide === false)"
                :style="{
                  width: obj.width ? obj.width : '150px',
                  textAlign: obj.textAlign ? obj.textAlign : 'left',
                  whiteSpace: 'nowrap',
                ...
                :class="obj?.headerClasses + ' ' + obj?.classes"
              >
                <div v-if="obj.footer == true">
                  <div v-if="obj.footerSumCurrency">
                    {{
                      currency(props.tableData.reduce((accumulator, val) => {
                        return accumulator + parseFloat(val[obj.key])
                      }, 0)  )
                    }}
                  </div>
                  <div v-if="obj.footerSum">
                    {{
          ...
          <tfoot v-if="props.tableData.length > 0">
            <tr class="border-bottom-primary">
              <template
                v-for="obj in showColumn.list.filter(
                  (item) => !item.hide || item.hide === false

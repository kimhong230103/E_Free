    ...
    <div class="modal-dialog modal-dialog-centered modal-md">
      <div class="modal-content">
        <div class="modal-header">
          <!-- <Icon name="solar:round-alt-arrow-left-bold" class="back btn" size="3rem" style="color: #253696;" /> -->

          <div data-bs-dismiss="modal" aria-label="Close" class="w-100 text-end cursor-pointer" @click="closeModal">
        ...
        <div class="modal-body p-l-25 p-r-25 py-4 pt-3">
          ...
          <h5 class="fw-bold mt-2 welcome_to">{{ $t("start_to_shopping_with") }} <span
              style="color: var(--theme-default);">E-Free</span></h5>
          <div class="row mt-1">
            <div class="col-12 form-log">
              <form action="">
                  ...
                  <div class="col-6">
                    <div class="form-group mb-2">
                      ...
                      <input v-model="form.first_name" type="text" id="first_name" class="form-control"
                        :placeholder="$t('first_name')" :class="{ 'is-invalid': v$.first_name.$error }"
                        @change="v$.first_name.$touch">
                      <span class="invalid-feedback" v-if="v$.first_name.$error">{{ v$.first_name.$errors[0].$message
                        }}</span>
                    </div>
                  </div>
                  <div class="col-6">
                    ...
                    <input v-model="form.last_name" type="text" id="last_name" class="form-control"
                      :placeholder="$t('lastname')" :class="{ 'is-invalid': v$.last_name.$error }"
                      @change="v$.last_name.$touch">
                    <span class="invalid-feedback" v-if="v$.last_name.$error">{{ v$.last_name.$errors[0].$message
                      }}</span>
                  </div>
                </div>
                <label for="dob" class="my-2 required">{{ $t("date_of_birth") }}</label>
                <div class="form-group date-picker-range w-100">
                  <IDatePicker v-model="form.dob" :multi-calendars="false" :date-only="true" :range="false"
                    :placeholder="$t('date_of_birth')" @dateChange="dateChange" :class="{ 'is-invalid': v$.dob.$error }"
                    @change="v$.dob.$touch" />
                ...
                <input v-model="form.confirm_password" type="password" id="confirm_password" class="form-control"
                  :placeholder="$t('confirm_password')" :class="{ 'is-invalid': v$.confirm_password.$error }"
                ...
                <div class="w-100 text-center">
                  ...
                  <p @click="signIn()" class="mt-2">{{ $t("already_have_an_account") }} <span class="text-underline"
                      style="color: #fb5d1f;">{{ $t("sign_in") }}</span></p>
                </div>
              </form>
            </div>
          </div>
